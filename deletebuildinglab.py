...
def delete_data(building_id, lab_id):
    try:
        connection = connect_db()
        cursor = connection.cursor()
        query = "DELETE FROM Building_Lab WHERE Building_ID = %s AND Lab_ID = %s"
        cursor.execute(query, (building_id, lab_id))
        connection.commit()
        if cursor.rowcount == 0:
            messagebox.showinfo("Info", f"No record found with Building ID {building_id} and Lab ID {lab_id}")
        else:
            messagebox.showinfo("Success", f"Record with Building ID {building_id} and Lab ID {lab_id} deleted successfully")
...
def delete_record():
    try:
        ...
        delete_data(building_id, lab_id)
    except ValueError:
...
def gui_setup():
    root = Tk()
    root.title("Delete Building Lab Record")
